...
export class SellerOrdersComponent implements OnInit {
  ...
  loadOrders(): void {
    ...
    this.orderService.mesCommandesVendeur().subscribe({
      next: (orders) => { this.orders = orders; this.applyFilters(); this.loading = false; this.cdr.detectChanges(); },
      error: () => { this.toastService.error('Erreur lors du chargement des commandes.'); this.loading = false; this.cdr.detectChanges(); }
    });
  }
  ...
  applyFilters(): void {
    ...
    this.currentPage = 1; this.cdr.detectChanges();
  }
  ...
  totalPages(): number { return Math.ceil(this.filteredOrders.length / this.pageSize); }
  ...
  getStatusLabel(status: string): string {
    const labels: { [key: string]: string } = { PENDING:'En attente', PAID:'Payée', PROCESSING:'En traitement', SHIPPED:'Expédiée', DELIVERED:'Livrée', CANCELLED:'Annulée' };
    return labels[status] || status;
  }

  getStatusClass(status: string): string {
    const classes: { [key: string]: string } = { PENDING:'badge-warning', PAID:'badge-info', PROCESSING:'badge-info', SHIPPED:'badge-accent', DELIVERED:'badge-success', CANCELLED:'badge-danger' };
