...
export class UserManagementComponent implements OnInit {
  ...
  loadUsers(): void {
    ...
    this.userService.listerUtilisateurs().subscribe({
      next: users => { this.allUsers = users; this.applyFilters(); this.loading = false; this.cdr.detectChanges(); },
  ...
  applyFilters(): void {
    ...
    this.filtered = this.allUsers.filter(u => {
      const matchSearch = !q || u.nom.toLowerCase().includes(q) || u.prenom.toLowerCase().includes(q) || u.email.toLowerCase().includes(q) || (u.telephone || '').includes(q);
      const matchRole = this.roleFilter === 'ALL' || u.role === this.roleFilter;
      const matchStatus = this.statusFilter === 'ALL' || (this.statusFilter === 'ACTIVE' && u.actif) || (this.statusFilter === 'INACTIVE' && !u.actif);
  ...
  pagedUsers(): UserResponse[] { const s = (this.currentPage - 1) * this.pageSize; return this.filtered.slice(s, s + this.pageSize); }
  ...
  openEdit(u: UserResponse): void {
    this.editingUser = u;
    this.editForm.patchValue({ nom: u.nom, prenom: u.prenom, email: u.email, telephone: u.telephone || '', role: u.role });
  ...
  saveEdit(): void {
    ...
    this.userService.modifierUtilisateur(this.editingUser.id, req).subscribe({
      next: updated => {
        ...
        this.applyFilters(); this.saving = false; this.closeEdit(); this.cdr.detectChanges();
      },
  ...
  toggleStatus(u: UserResponse): void {
    const obs = u.actif ? this.userService.desactiverCompte(u.id) : this.userService.activerCompte(u.id);
    obs.subscribe({
      next: updated => { const idx = this.allUsers.findIndex(x => x.id === updated.id); if (idx !== -1) this.allUsers[idx] = updated; this.applyFilters(); this.cdr.detectChanges(); }
    });
  }
  ...
  executeDelete(): void {
    ...
    this.userService.supprimerUtilisateur(this.deletingUser.id).subscribe({
      next: () => { this.allUsers = this.allUsers.filter(u => u.id !== this.deletingUser!.id); this.applyFilters(); this.deleting = false; this.cancelDelete(); this.cdr.detectChanges(); },
      error: () => { this.deleting = false; this.cdr.detectChanges(); }
    });
  }

  roleLabel(r: string): string { return ({ CUSTOMER:'Client', SELLER:'Vendeur', ADMIN:'Admin' } as any)[r] || r; }
  roleBadgeClass(r: string): string { return ({ CUSTOMER:'badge-info', SELLER:'badge-accent', ADMIN:'badge-warning' } as any)[r] || 'badge-muted'; }
  initials(u: UserResponse): string { return `${u.prenom.charAt(0)}${u.nom.charAt(0)}`.toUpperCase(); }
  avatarColor(u: UserResponse): string {
    const colors = ['#6366f1','#8b5cf6','#ec4899','#f59e0b','#10b981','#3b82f6','#ef4444'];
    return colors[(u.id || 0) % colors.length];
  }

  totalActive(): number { return this.allUsers.filter(u => u.actif).length; }
  totalCustomers(): number { return this.allUsers.filter(u => u.role === 'CUSTOMER').length; }
  totalSellers(): number { return this.allUsers.filter(u => u.role === 'SELLER').length; }
  totalAdmins(): number { return this.allUsers.filter(u => u.role === 'ADMIN').length; }
}
