...
import { ProductRequest, ProductResponse } from 'src/Models/product.model';
import { environment } from 'src/environments/environment';

// Service produits - requêtes HTTP vers l'API produits
...
export class ProductService {
  ...
  listerProduits(page = 0, size = 12, sort = 'dateCreation'): Observable<Page<ProductResponse>> {
    const params = new HttpParams()
      ...
      .set('size', size)
      .set('sort', sort + ',desc');
    return this.httpClient.get<Page<ProductResponse>>(this.API, { params });
  }
  ...
  voirProduit(id: number): Observable<ProductResponse> {
    return this.httpClient.get<ProductResponse>(`${this.API}/${id}`);
  }

  rechercher(q: string, page = 0, size = 12): Observable<Page<ProductResponse>> {
    const params = new HttpParams().set('q', q).set('page', page).set('size', size);
    return this.httpClient.get<Page<ProductResponse>>(`${this.API}/search`, { params });
  }

  topVentes(): Observable<ProductResponse[]> {
    return this.httpClient.get<ProductResponse[]>(`${this.API}/top-selling`);
  }

  statistiquesAccueil(): Observable<HomeStats> {
    return this.httpClient.get<HomeStats>(`${this.API}/stats`);
  }

  promos(page = 0, size = 12): Observable<Page<ProductResponse>> {
    const params = new HttpParams().set('page', page).set('size', size);
    return this.httpClient.get<Page<ProductResponse>>(`${this.API}/promos`, { params });
  }

  creerProduit(request: ProductRequest): Observable<ProductResponse> {
    return this.httpClient.post<ProductResponse>(this.API, request);
  }

  modifierProduit(id: number, request: ProductRequest): Observable<ProductResponse> {
    return this.httpClient.put<ProductResponse>(`${this.API}/${id}`, request);
  }
  ...
  mesProduits(page = 0, size = 12): Observable<Page<ProductResponse>> {
    const params = new HttpParams().set('page', page).set('size', size);
    return this.httpClient.get<Page<ProductResponse>>(`${this.API}/my`, { params });
  }
}
